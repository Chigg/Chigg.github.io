...
This is an example output from QGIS using GE Satellite imagery. As you can see, naturally, the Gates-Dell Complex is popping for instagram pics. This was really just coincidental, as the way it's simulated is more or less just rolling a dice. The red marked areas are locations with the highest average of likes. It's important to note the precision of the graphs, as with this visualization it's easy to underestimate. The actual point for each location is based on a gps coordinate, the circles just get blown out when you zoom out, in order to get a better picture of the location. This can be changed within QGIS but I thought it was more appealing for this specific purpose. I filtered the data to ignore about 80% of the locations, so that we could only see the highest. Another interesting way to visualize this data might be to generate a heat map. There is a fairly popular heatmap plugin for QGIS which I've seen a lot of people use. However, I didn't think I had enough "data" to warrant the effort.
